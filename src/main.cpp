...
// Landscape world size after setRotation(1) on 240x320 ILI9341 (Obsidian ES3N28P)
...
// ILI9341 BGR is handled by TFT_eSPI; set true if red/blue look swapped
...
const int MAX_PLANTS = 70;
const int MAX_HERBS = 55;
const int MAX_CARNS = 10;
const int MAX_APEX = 4;
const int MAX_DECOMPS = 14;
const int MAX_SPORES = 20;
const int MAX_PLANKTON = 140;
const int MAX_PARTICLES = 180;
const int MAX_GARBAGES = 40;
...
void spawnPlant() {
  for(int i=0; i<MAX_PLANTS; i++) {
    if(!plants[i].active) {
      ...
      plants[i].x = random(5, SIM_WIDTH - 5); plants[i].y = random(5, SIM_HEIGHT - 5);
      break;
    }
  }
}
...
void spawnHerb(float x, float y, float pSpeed = 0.8f) {
  for(int i=0; i<MAX_HERBS; i++) {
    if(!herbs[i].active) {
      herbs[i].active = true;
      herbs[i].x = (x == -1) ? random(10, SIM_WIDTH-10) : x + random(-5, 5);
      herbs[i].y = (y == -1) ? random(10, SIM_HEIGHT-10) : y + random(-5, 5);
...
void spawnCarn(float x, float y, float pSpeed = 1.1f) {
  for(int i=0; i<MAX_CARNS; i++) {
    if(!carns[i].active) {
      carns[i].active = true;
      carns[i].x = (x == -1) ? random(10, SIM_WIDTH-10) : x + random(-5, 5);
      carns[i].y = (y == -1) ? random(10, SIM_HEIGHT-10) : y + random(-5, 5);
...
void spawnApex(float x, float y, float pSpeed = 1.5f) {
  for(int i=0; i<MAX_APEX; i++) {
    if(!apex[i].active) {
      apex[i].active = true;
      apex[i].x = (x == -1) ? random(10, SIM_WIDTH-10) : x + random(-5, 5);
      apex[i].y = (y == -1) ? random(10, SIM_HEIGHT-10) : y + random(-5, 5);
...
void spawnDecomp(float x, float y) {
  for(int i=0; i<MAX_DECOMPS; i++) {
    if(!decomps[i].active) {
      ...
      decomps[i].x = (x == -1) ? random(10, SIM_WIDTH-10) : x + random(-5, 5);
      decomps[i].y = (y == -1) ? random(10, SIM_HEIGHT-10) : y + random(-5, 5);
...
void core0Task(void * pvParameters) {
  for(;;) {
    ...
    if (random(0, 10000) < 10) spawnSpore(random(10, SIM_WIDTH-10), random(10, SIM_HEIGHT-10));
    ...
    for(int i=0; i<MAX_DECOMPS; i++) {
      ...
      if(decomps[i].x > SIM_WIDTH) { decomps[i].x = SIM_WIDTH; decomps[i].vx *= -1; }
      if(decomps[i].y < 0) { decomps[i].y = 0; decomps[i].vy *= -1; }
      if(decomps[i].y > SIM_HEIGHT) { decomps[i].y = SIM_HEIGHT; decomps[i].vy *= -1; }
    ...
    for(int i=0; i<MAX_SPORES; i++) {
      ...
      if(spores[i].x > SIM_WIDTH) spores[i].x = 0; if(spores[i].x < 0) spores[i].x = SIM_WIDTH;
      if(spores[i].y > SIM_HEIGHT) spores[i].y = 0; if(spores[i].y < 0) spores[i].y = SIM_HEIGHT;
    ...
    for(int i=0; i<MAX_HERBS; i++) {
      ...
      if(herbs[i].x > SIM_WIDTH) { herbs[i].x = SIM_WIDTH; herbs[i].vx *= -1; }
      if(herbs[i].y < 0) { herbs[i].y = 0; herbs[i].vy *= -1; }
      if(herbs[i].y > SIM_HEIGHT) { herbs[i].y = SIM_HEIGHT; herbs[i].vy *= -1; }
    ...
    for(int i=0; i<MAX_CARNS; i++) {
      ...
      if(carns[i].x > SIM_WIDTH) { carns[i].x = SIM_WIDTH; carns[i].vx *= -1; }
      if(carns[i].y < 0) { carns[i].y = 0; carns[i].vy *= -1; }
      if(carns[i].y > SIM_HEIGHT) { carns[i].y = SIM_HEIGHT; carns[i].vy *= -1; }
    ...
    for(int i=0; i<MAX_APEX; i++) {
      ...
      if(apex[i].x > SIM_WIDTH) { apex[i].x = SIM_WIDTH; apex[i].vx *= -1; }
      if(apex[i].y < 0) { apex[i].y = 0; apex[i].vy *= -1; }
      if(apex[i].y > SIM_HEIGHT) { apex[i].y = SIM_HEIGHT; apex[i].vy *= -1; }
...
void setup() {
  ...
  img.setAttribute(PSRAM_ENABLE, true); // ~150 KB full-frame sprite needs PSRAM
  img.createSprite(SIM_WIDTH, SIM_HEIGHT);
  ...
  for(int i=0; i<55; i++) spawnPlant();
  for(int i=0; i<35; i++) spawnHerb(-1, -1);
  for(int i=0; i<4; i++) spawnCarn(-1, -1);
  for(int i=0; i<2; i++) spawnDecomp(-1, -1);
  
  // プランクトン（パララックスレイヤー）
  for(int i=0; i<MAX_PLANKTON; i++) {
    planktons[i].x = random(0, SIM_WIDTH);
    planktons[i].y = random(0, SIM_HEIGHT);
...
void loop() {
  ...
  for(int i=0; i<MAX_PLANKTON; i++) {
    ...
    if(planktons[i].x > SIM_WIDTH) planktons[i].x = 0;
