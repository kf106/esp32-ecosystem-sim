...
- **Eat sound effects**: Short distinct chomps on the ES8311 speaker when anything is eaten.
...
| Audio | ES8311 codec + FM8002E amp (external speaker on 1.25 mm horn jack) |
...
Plug a small speaker into the board's **horn** connector (1.25 mm 2-pin). Amp enable is GPIO 1 (active low).

On boot you should hear a short **880 Hz beep**. Press **BOOT** to mute/unmute (high chirp = on, low chirp = off). Serial shows `sfx: on` / `sfx: muted`.

If the boot beep is silent, check the speaker jack first; serial should show `sfx: ready` and `sfx: ES8311 REG00=0x..`.

Eat events play soft embedded blips (no SD card), rate-limited (~450 ms):

| Event | Sound |
|-------|--------|
| Herbivore eats plant | soft high tick |
| Herbivore dies | soft mid blip (once) |
| Carnivore dies | soft low blip (once) |
| Decomposer eats garbage | muted thup |
| Decomposer eats spore | quiet airy rise |

Continuous predation nibbles are silent (they were the Space-Invaders spam).

Clips are generated by [`tools/gen_sfx.py`](tools/gen_sfx.py) into [`src/sfx_data.h`](src/sfx_data.h). Re-run the script only if you change the generator.
